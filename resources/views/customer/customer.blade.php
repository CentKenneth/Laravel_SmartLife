<x-customer_dashboard>
    <header class="bg-white shadow">
        <div class="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
          <h1 class="text-3xl font-bold tracking-tight text-gray-900">Customer Dashboard</h1>
          <p class="mt-2 text-sm text-gray-700">A list of all the summary data from users</p>
        </div>
      </header>
      <div class="px-12 mt-6">
        <h3 class="text-base font-semibold leading-6 text-gray-900">Summary</h3>
        {{-- <dl class="mt-5 grid grid-cols-1 gap-5 sm:grid-cols-3">
            <div v-for="item in stats" class="overflow-hidden rounded-lg bg-white px-4 py-5 shadow-2xl sm:p-6">
            ...
            <div v-for="item in stats" class="overflow-hidden rounded-lg bg-white px-4 py-5 shadow-2xl sm:p-6">
                ...
                <dd class="mt-1 text-3xl font-semibold tracking-tight text-gray-900">{{ $totalTransactions }}</dd>
            </div>
            <div v-for="item in stats" class="overflow-hidden rounded-lg bg-white px-4 py-5 shadow-2xl sm:p-6">
